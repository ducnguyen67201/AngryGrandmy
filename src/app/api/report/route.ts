...
import { shouldReuseFixJob } from "@/lib/fixes/fix-job-cache";
...
async function createFixJob(input:z.infer<typeof FixRequestSchema>){
  const id=`fix-${input.sessionId}-${input.frustrationEventId}`.replace(/[^a-zA-Z0-9-]/g,"-").slice(0,180);
  const file=path.resolve(process.env.GRANNYSMITH_FIX_JOB_FILE??".grannysmith/fix-jobs.json");
  const desiredMode=process.env.GRANNYSMITH_FIX_AGENT_MODE==="codex"?"codex":"fallback";
  let jobs:Record<string,unknown>[]=[];
  try{jobs=JSON.parse(await fs.readFile(file,"utf8"))}catch{}
  const existing=jobs.find(j=>j.id===id);
  if(shouldReuseFixJob(existing,desiredMode))return existing;
  const fallback=`${input.frustration.recommendation}\n\nEvidence: ${input.frustration.visibleEvidence}\n\nAdd a regression test for ${input.personaName}.`;
  const roles=["investigator","fix-proposer"];
  const proposals=desiredMode==="codex"
    ?await Promise.all(roles.map(async role=>({role,details:await runCodex(path.resolve(process.env.GRANNYSMITH_REPO_PATH??process.cwd()),`Act as ${role}. Read only; do not modify, commit, push, or contact external systems. Investigate: ${input.frustration.observation}. Evidence: ${input.frustration.visibleEvidence}. Propose a fix and tests.`)})))
    :[{role:"fix-proposer",details:fallback}];
  const job={...input,id,status:"completed",mode:desiredMode,proposals,createdAt:new Date().toISOString()};
  await fs.mkdir(path.dirname(file),{recursive:true});
  const retained=jobs.filter(item=>item.id!==id);
  await fs.writeFile(file,JSON.stringify([...retained,job],null,2),{mode:0o600});
  return job;
}
